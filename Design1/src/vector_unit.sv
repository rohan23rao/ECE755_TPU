module vector_unit #(
    ...
    // Converted from unpacked: logic [FP16_WIDTH-1:0] data_in [0:LANES-1]
    input logic  [LANES-1:0][FP16_WIDTH-1:0] data_in,   // 8 FP16 input data
    input logic  [FP16_WIDTH - 1:0] scale_data,          // 1 FP16 data for one cycle, quantize column by column

    // Converted from unpacked: logic [FP4_WIDTH-1:0] data_out [0:LANES-1]
    output logic [LANES-1:0][FP4_WIDTH-1:0] data_out     // 8 FP4 quantized output data
);

// Converted from unpacked: logic [FP16_WIDTH-1:0] quant_in [LANES-1:0]
logic [LANES-1:0][FP16_WIDTH-1:0] quant_in;    // Muxed input for quantization
// Converted from unpacked: logic [FP16_WIDTH-1:0] quant_scale [LANES-1:0]
logic [LANES-1:0][FP16_WIDTH-1:0] quant_scale; // Muxed scale for quantization
// Converted from unpacked: logic [FP4_WIDTH-1:0] reg_out [LANES-1:0]
logic [LANES-1:0][FP4_WIDTH-1:0]  reg_out;     // Register to hold quantized output after RELU
// Converted from unpacked: logic [FP4_WIDTH-1:0] mult_out [LANES-1:0]
