...
logic  [FP16_WIDTH-1:0] mult_out   [LANES - 1:0];      // Output of the FP16 multiplication before quantization
...
generate 
    for (i=0; i<LANES; i=i+1) begin : gen_scale_data_reg
       ...
       FloatP16x16 iFloat (.A(quant_in[i]), .B(quant_scale[i]), .Out(mult_out[i])); // FP16 multiplier instance for quantization scaling

       // RELU clamping & Quantization logic
       always_ff @(posedge clk) begin
         if (quant_en[i])
                reg_out[i] <= {1'b0, mult_out[i][14:13], mult_out[i][9]}; // take sign bit, MSB of mantissa, and 2 MSBs of exponent for FP4 E2M1
        end
