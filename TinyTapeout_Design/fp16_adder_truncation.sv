module fp16_adder_truncation (
    ...
    // Compare and swap to larger/smaller
    logic        swap;
    logic        s_lg,   s_sm;
    logic [5:0]  e_lg,   e_sm;
    ...
    // Near path: effective subtraction with exp_diff <= 1
    logic use_near;
    ...
    ks_sub14 u_near_sub (.a(near_lg_ext), .b(near_sm_ext), .diff(near_diff));

    logic [3:0] near_lzc;
    logic       near_all_zero;
    lod_tree_14 u_near_lzc (.din(near_diff), .lzc(near_lzc), .all_zero(near_all_zero));
    ...
    assign far_post_exp  = (!far_trunc_sig[10])    ? 6'd0 :
                           (far_norm_exp == 6'd0)   ? 6'd1 : far_norm_exp;
    ...
    // Output mux (is_underflow removed: maps to is_normal, outputs signed zero)
    logic is_both_zero, is_only_a_zero, is_only_b_zero, is_overflow, is_normal;
    ...
    assign is_normal      = ~is_both_zero & ~is_only_a_zero & ~is_only_b_zero & ~is_overflow;
